...
Given two integers n, m and a 2D matrix mat of dimensions nxm, the task is to find
...
findMaxSum() which takes the two integers n, m, and the 2D matrix mat as input parameters
and returns the maximum sum of an hourglass in the matrix. If there are no hourglasses, 
it returns -1.
...
public class HourGlassMaxSum
{
    ...
    static int findMaxSum(int n, int m, int a[][]) {
        int res=-1;
        for(int i=0;i<n-2;i++){
            for(int j=0;j<m-2;j++){
