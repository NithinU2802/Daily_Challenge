...
Your Task:
You do not have to take any input or print anything. The task is to complete the
function splitArray() which returns the maximum sum subarray after splitting the 
array into K subarrays such that maximum sum subarray is minimum possible.
...
public class SplitArray{ 
    public static int splitArray(int[] a,int n,int K){
        int i=0,j=0;
        for(int k:a){
            if(k>i)
                i=k;
	...
	public static void main(String[] args) {
		...
		for(int i=0;i<n;i++)
		    a[i]=x.nextInt();
		System.out.println(splitArray(a,n,k));
	}
}
