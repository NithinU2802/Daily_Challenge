...
then we will say that u depends on v. Number of Dependencies (NoD) for a node x is
the total count of nodes that x depends upon. Find out the sum of number of dependencies
of every node.
...
sumOfDependencies() which takes the adj (Adjacency list) and V (Number of nodes) as 
input parameters and returns the total sum of Number of Dependencies of all nodes.
...
public class SumOfDependencies
{
    ...
    public static int sumOfDependencies(ArrayList<ArrayList<Integer>> adj,int v){
        int s=0;
